/*   two sum.
두 개의 음수가 아닌 정수를 나타내는 두 개의 비어 있지 않은 링크 목록이 제공됩니다.
숫자는 역순으로 저장되고 각 노드에는 한 자릿수가 포함됩니다. 
두 숫자를 추가하고 합계를 연결된 목록으로 반환합니다.

숫자 0을 제외한 두 숫자의 리드 0이 포함되어 있지 않다고 가정할 수 있습니다.

    ?! 스택을 이용해봄
    로직
*   1. 1배열 2배열을 차례로 꺼내어 스택에 넣는다
    2. 스택에서 하나씩 꺼내서 숫자를 만들고 더하기로 결과값을 구함
    3. 결과값을 다시 스택에 하나씩 넣고 꺼내 링크드 리스트로 완성
*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
